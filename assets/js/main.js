...
(() => {
  ...
  /* Video header: cuba main DENGAN BUNYI dari mula; jika pelayar sekat,
     jatuh balik ke senyap dan buka bunyi pada interaksi pertama pengguna */
  ...
  if (heroVid && !reduceMotion) {
    heroVid.volume = 0.85;
    let userMuted = false; // pengguna sengaja tekan mute — jangan buka semula

    const setIcon = () => {
      if (!sndBtn) return;
      sndBtn.classList.toggle("muted", heroVid.muted);
      sndBtn.setAttribute("aria-label", heroVid.muted ? "Buka bunyi video" : "Matikan bunyi video");
      sndBtn.title = heroVid.muted ? "Buka bunyi" : "Matikan bunyi";
    };

    const armAutoUnmute = () => {
      const onFirst = (e) => {
        if (e.target && e.target.closest && e.target.closest("#snd-toggle")) return; // biar butang uruskan
        off();
        if (!userMuted) {
          heroVid.muted = false;
          heroVid.play().catch(() => {});
      ...
      const off = () => ["pointerdown", "keydown", "touchstart"].forEach((t) =>
        window.removeEventListener(t, onFirst, true));
      ["pointerdown", "keydown", "touchstart"].forEach((t) =>
        window.addEventListener(t, onFirst, { capture: true, passive: true }));
    };

    heroVid.muted = false;
    const attempt = heroVid.play();
    if (attempt && attempt.catch) {
      attempt.then(setIcon).catch(() => {
        heroVid.muted = true;
        heroVid.play().catch(() => {});
        setIcon();
        armAutoUnmute();
    ...
    if (sndBtn) {
      sndBtn.addEventListener("click", () => {
        ...
        userMuted = heroVid.muted;
        if (!heroVid.muted && heroVid.paused) heroVid.play().catch(() => {});
        setIcon();
      });
    }
  } else if (sndBtn) {
